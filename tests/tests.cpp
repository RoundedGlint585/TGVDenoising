...
TEST_F(GPUImageTest, gpuEnergy) {
    gpu::gpu_mem_32f atomsBuf, chargesBuf, resultBuf;
    gpu::gpu_mem_32u bondsBuf, indexBuf;
    EnergyKernel.compile();
    ...
    atomsBuf.resizeN(atoms.size());
    atomsBuf.writeN(atoms.data(), atoms.size());
    ...
    chargesBuf.resizeN(charges.size());
    chargesBuf.writeN(charges.data(), charges.size());
    ...
    bondsBuf.resizeN(linedBonds.size());
    bondsBuf.writeN(linedBonds.data(), linedBonds.size());
    indexBuf.resizeN(indexingBonds.size());
    indexBuf.writeN(indexingBonds.data(), indexingBonds.size());
    resultBuf.resizeN(result.size());
    std::cout << (uint32_t) charges.size() << " " << (uint32_t) bonds.size() << " "
              << (uint32_t) charges.size() * (uint32_t) charges.size() << std::endl;
    unsigned int workGroupSize = 128;
    unsigned int globalWorkSize = (charges.size() * charges.size() + workGroupSize - 1) / workGroupSize * workGroupSize;
    gpu::WorkSize workSize = gpu::WorkSize(workGroupSize, globalWorkSize);
    EnergyKernel.exec(gpu::WorkSize(workGroupSize, globalWorkSize), chargesBuf, bondsBuf, indexBuf, atomsBuf, resultBuf,
                      (uint32_t) charges.size(), (uint32_t) bonds.size(),
                      (uint32_t) charges.size() * (uint32_t) charges.size());
    float E = 0.0f;
    resultBuf.readN(result.data(), result.size());
    for (auto &i: result) {
        E += i;
    }
    std::cout << E * 1389.38757f << std::endl;
}
